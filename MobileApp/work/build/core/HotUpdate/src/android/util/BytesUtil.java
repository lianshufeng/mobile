...
public class BytesUtil {
	...
	public static int findLast(byte[] bin, byte[] queryBin) {
		...
		for (int i = 0; i < total; i++) {
			...
			byte[] target = subBytes(bin, lastIndex, queryBin.length
					+ lastIndex);
	 ...
	 * @return
	 */
	public static byte[] subBytes(final byte[] bin, final int start,
			final int end) {
	...
	public static byte[] hexToBin(String str) throws IOException {
		...
		for (int i = 0; i < hex.length() / 2; i++) {
			int point = i * 2;
			byteArrayOutputStream.write(Integer.parseInt(
					hex.substring(point, point + 2), 16));
		}
