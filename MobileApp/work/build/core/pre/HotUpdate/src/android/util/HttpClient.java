...
public class HttpClient {
	...
	public void setListenOutputStream(OutputStream listenOutputStream) {
	 ...
	 * @throws IOException
	 */
	public ResultBean ReadDocuments(String url, boolean isPost, byte[] postData)
			throws IOException {
		return ReadDocuments(new URL(url), isPost, postData, null);
	}
	 ...
	 * @throws IOException
	 */
	public ResultBean ReadDocuments(String url, boolean isPost,
			byte[] postData, Map<String, String> requestHead)
			throws IOException {
		return ReadDocuments(new URL(url), isPost, postData, requestHead);
	}

	public ResultBean ReadDocuments(final URL url, final boolean isPost,
			final byte[] postData, Map<String, String> requestHead)
			throws IOException {
		...
		// 设置响应与返回的响应头信息
		ResultBean resultBean = readInputStream(httpURLConnection);
		// 设置服务器需要设置的cookies
	 ...
	 * @throws IOException
	 */
	protected HttpURLConnection connect(final URL url, final Boolean isPost,
			Map<String, String> requestHead) throws IOException {
		URLConnection uRLConnection = url.openConnection();
		HttpURLConnection httpURLConnection = (HttpURLConnection) uRLConnection;
		// 设置是否自动重定向
		httpURLConnection.setInstanceFollowRedirects(isFollowRedirects());
		// 设置访问模式
		httpURLConnection.setRequestMethod(isPost != null && isPost ? "POST"
				: "GET");
	 ...
	 * @throws IOException
	 */
	protected ResultBean readInputStream(HttpURLConnection httpURLConnection)
			throws IOException {
		// 读取响应的数据
		...
		if (httpURLConnection.getHeaderField("Content-Length") != null) {
			totalByte = Long.parseLong(httpURLConnection
	 ...
	 * @return
	 */
	private byte[] inputStreamToBytes(InputStream inputStream, long totalByte)
			throws IOException {
	...
	public class ResultBean implements Serializable {
		...
		public String getCharset() {
			String charset = null;
			// 取出文字编码
			List<String> contentTypes = this.getResponseHead().get(
					"Content-Type");
